...
class Punktuator:
	...
	def processInputTextWithTimings(self, data):
		...
		for line in data:
			...
			timingString = re.search("^([0-9:,\.]*?)\r?\n",line)
			if timingString != None:
				currentTiming = timingString.group(1)			
				if self.config["debug"]: print("Reading timing {:s}".format(currentTiming))
				
			# Otherwise, put the strings into the current timing.
			if currentTiming in result:
